...
export async function POST(request: NextRequest) {
  try {
    ...
    if (totalAmount <= 0) {
      return NextResponse.json({ error: "No balance to pay out" }, { status: 400, headers: corsHeaders });
    }
    ...
    try {
        ...
        if (senderBalance === null || senderBalance < totalAmount) {
            return NextResponse.json({ error: "Insufficient funds in the payout wallet." }, { status: 503 });
        }
    } catch (error) {
        return NextResponse.json({ error: "Could not verify the payout wallet's balance." }, { status: 500 });
    }
